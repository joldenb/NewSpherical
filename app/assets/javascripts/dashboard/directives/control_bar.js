...
.directive('topicCntrlBtn', ['$state', 'ActivityVis', 'UserInfo', 'ChooserData', function($state, ActivityVis, UserInfo, ChooserData) {
    return {
        restrict: 'A',
        scope: {
            btntarget: '@'
...
.directive('cntrlbarBtn', ['$compile', '$state', 'ActivityVis', 'UserInfo', 'ChooserData', function($compile, $state, ActivityVis, UserInfo, ChooserData) {
    return {
        ...
        link: function(scope, elm, attrs) {
            var activityController = scope.$parent,
            // newDiscussion = function() {
            //     if ((!$state.includes('**.topic.**') && !activityController.channelActive)  || ActivityVis.shareitem) {
            //         return;
            //     }
            //     UserInfo.signedin().then(function(d) {
            //         if (d.signedin) {
